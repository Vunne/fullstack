...
def deleteMatches():
	...
	db = connect()
	c = db.cursor()
	c.execute("delete from matches where winner >= 0;")
	db.commit()
	db.close()
...
def playerStandings():
	...
	db = connect()
	c = db.cursor()
	c.execute("""select id, name, count(winner) as wins, 
		...
		group by id order by wins desc, name;""")
	standings = []
	for row in c.fetchall():
...
def reportMatch(winner, loser):
	...
	c.execute("insert into matches (winner, loser) values (%s, %s);", 
		((bleach.clean(winner, strip=True),), (bleach.clean(loser, strip=True),)))
	db.commit()
	db.close()
